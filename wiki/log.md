...
## [2026-07-14] ruling | Binder gets Momentum hooks and joins the Weapon Mastery Non-Martials tier
- Trigger: two remaining MED-severity items from the class-system audit — Binder was the only class with zero Momentum interaction (despite Bleed Points sharing near-identical gain triggers) and the only class confirmed to have zero Weapon Mastery access at all. User: "Binder needs some momentum aspects and [figure out] what weapon mastery makes sense."
- **Momentum resolved**: two original hooks added (not in `.raw/Binder.md`, `.raw/` untouched). **Borrowed Rhythm** — spend a Bleed Point on Push the Bind, and if the triggered ability succeeds, gain +1 Momentum (mirrors Barbarian's spend-plus-success counterattack pattern). Renamed from an earlier drafted "Momentum Surge" after discovering that name is already [[Sorcerer]]'s (Volatile Aura, 3rd level) — avoided recreating the exact naming-collision problem just fixed for Exploit Weakness/Vulnerability. **Cracked Vessel** — when Momentum resets to 0 from a Major Injury or Fatal/0 HP threshold, gain 1 Bleed Point, tying Binder's "wear becomes power" identity to Momentum's own harshest loss event.
- **Weapon Mastery resolved**: previously read as total exclusion from the subsystem. On review, Binder's silence is the identical shape that already confirmed Sangromancer, Warlock, and Witch as **Non-Martials** (5 crits with a simple weapon unlocks mastery, martial only via the Weapon Adept feat) rather than full exclusion — no principled reason existed to treat Binder's silence differently. Binder's "Weapons: simple" proficiency and starting equipment match that row exactly.
- **Pages updated**: [[Binder]] (both sections rewritten with the resolution + original-addition flags), [[Weapon Mastery]] (table row + resolution note), [[Weapon Mastery System]] (Binder mention resolved), [[Momentum]] (Binder's "total absence" paragraph updated), [[FEARS MOC]] (three separate mentions struck through and resolved).
- Note: this closes both remaining MED-severity Binder items from the original audit. Binder is no longer the roster's one structurally incomplete class.

## [2026-07-14] ruling | Warden's Last Stand DC gap closed (10+PB+WIS); Ranger's "gap" ruled a non-issue
- Trigger: fourth and final HIGH-severity item from the class-system audit's contradiction ledger. [[Warden]]'s The Earth Remembers names a save type (DEX/CON per Prime Element) across all five variants but gives no DC anywhere; [[Ranger]]'s three Last Stand options state no DC either. User asked whether a general Last Stand formula already exists that could just be applied.
- **Investigation**: [[Last Stand]]'s DC-base section already establishes `10+PB+ability` as canonical, confirmed by well over a dozen data points across the roster, with "ability" consistently matching each class's own primary casting stat. Checked Ranger's actual three options (Hunter's Reckoning, Call of the Wild, Ghost of the Vale) and found none of them include a target saving throw at all — auto-hit, summon, and buff/aura effects only.
- **Resolved**: Warden's gap was genuine and is now closed — `DC = 10+PB+WIS` (matching Warden's own confirmed `8+PB+WIS` spellcasting DC), applied uniformly across all five Earth Remembers variants. Ranger's "gap" was never a gap — its capstone kit simply doesn't use save-or-suffer effects, so there's no DC to add.
- **Pages updated**: [[Warden]] (DC filled in, resolved callout added), [[Last Stand]] (DC-base section, both per-class option tables for Warden and Ranger), [[Ranger]] (note clarified as resolved-non-issue), [[FEARS MOC]] (open item struck through and resolved).
- Note: this closes the fourth and final HIGH-severity item from the class-system audit's contradiction ledger. All four are now resolved.
